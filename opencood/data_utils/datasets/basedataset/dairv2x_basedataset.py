...
class DAIRV2XBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        few_shot_rate = params.get('few_shot_rate', None)
        if few_shot_rate is None:
            self.split_info = read_json(split_dir)
        elif isinstance(few_shot_rate, float):
            self.split_info = read_json(split_dir)
            len_split_info = len(self.split_info)
            self.split_info = self.split_info[:int(few_shot_rate * len_split_info)]
            print(f"# few_shot_rate is {few_shot_rate}", f"len of samples is {len(self.split_info)}/{len_split_info}")
