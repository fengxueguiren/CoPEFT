...
class OPV2VBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True):
        ...
        self.few_shot_rate = params.get('few_shot_rate', None)
        
        self.sample_numbers = list()
        self.all_numbers = list()
        self.reinitialize()


        if self.few_shot_rate is not None:
            print(f"# few_shot_rate is {self.few_shot_rate}", f"len of samples is {sum(self.sample_numbers)}/{sum(self.all_numbers)}")
    ...
    def reinitialize(self):
        ...
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                len_split_info = len(yaml_files)
                sample_len_split_info = len_split_info
                if isinstance(self.few_shot_rate, float):
                    yaml_files = yaml_files[:int(self.few_shot_rate * len_split_info)]

                    sample_len_split_info = len(yaml_files)
                    # print(f"# few_shot_rate is {self.few_shot_rate}", f"len of samples is {len(yaml_files)}/{len_split_info}")
                
                self.all_numbers.append(len_split_info)
                self.sample_numbers.append(sample_len_split_info)
