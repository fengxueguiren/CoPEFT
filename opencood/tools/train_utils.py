...
def make_saved_path(saved_path, dir_name=None, time_mark=True, hypes=None, fthypes=None):
    current_time = datetime.now()
    time_dir_name = current_time.strftime("_%Y_%m_%d_%H_%M_%S")

    if time_mark is True:
        full_path = os.path.join(saved_path, dir_name+time_dir_name)
    else:
        full_path = os.path.join(saved_path, dir_name)
    ...
    if not os.path.exists(full_path):
        if not os.path.exists(full_path):
            try:
                ...
                pass
        save_name = os.path.join(full_path, 'fine_tuning_config.yaml')
        with open(save_name, 'w') as outfile:
            yaml.dump(fthypes, outfile)
...
def setup_train(hypes, sub_dir=None):
    """
    ...
    if sub_dir is None:
        ...
        full_path = os.path.join(current_path, sub_dir, folder_name)
...
def create_model(hypes):
    ...
    backbone_config['max_cav'] =  hypes['train_params']['max_cav']
...
def setup_optimizer(hypes, model, param_groups=None):
    """
    ...
    if param_groups is None:
        ...
        if 'args' in method_dict:
            return optimizer_method(param_groups,
                                    **method_dict['args'])
        else:
            return optimizer_method(param_groups)
