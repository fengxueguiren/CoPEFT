...
def train_parser():
    ...
    parser.add_argument("--hypes_yaml", "-y", type=str, required=False,
                        default='/data/code/CoPEFT/opencood/hypes_yaml/dairv2x/baselines/pointpillar_focal.yaml',
                        help='data generation yaml file needed ')
    parser.add_argument('--model_dir', default='',
                        help='Continued training path')
    parser.add_argument('--fusion_method', '-f', default="intermediate",
                        help='passed to inference.')
    parser.add_argument('--few_shot_rate', default=-1.0, type=float,
...
def main():
    ...
    if hypes['fusion']['dataset'] in ['dairv2x', 'opv2v', 'v2xset'] and opt.few_shot_rate != -1:
    ...
    if opt.model_dir:
        ...
        if few_shot_rate is None:
            ...
            saved_path = train_utils.setup_train(hypes, sub_dir="rate_"+str(few_shot_rate))
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            if supervise_single_flag:
                ...
                criterion.logging(epoch, i, len(train_loader), writer, suffix="_single", pbar=pbar)
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    pbar_val.update(1)
                    criterion.logging(epoch, i, len(train_loader), writer=None, suffix="_eval", pbar=pbar_val)
    ...
    if run_test:
        ...
        if 'noise_setting' in hypes and hypes['noise_setting']['add_noise']:
            cmd = f"python /data/code/CoPEFT/opencood/tools/inference_w_noise.py --model_dir {saved_path} --fusion_method {fusion_method}"
        else:
            cmd = f"python /data/code/CoPEFT/opencood/tools/inference.py --model_dir {saved_path} --fusion_method {fusion_method}"
