...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if CoT_flag is True and inplanes == planes and stride == 1:
            self.conv1 = CoTAttention(inplanes)
        else:
            self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = norm_layer(planes)
        self.relu = nn.ReLU(inplace=True)
        if CoT_flag is True and inplanes == planes:
            self.conv2 = CoTAttention(inplanes)
        else:
