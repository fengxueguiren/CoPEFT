...
class AttFusion(nn.Module):
    ...
    def forward(self, xx, record_len, normalized_affine_matrix):
        ...
        for b in range(B):
            ...
            # update each node i
            i = 0 # ego
            # print(f'batch_node_features[b]={batch_node_features[b].shape}, t_matrix[i, :, :, :]={t_matrix[i, :, :, :].shape}, (H, W)={(H, W)}')
...
class AttFusion_deep(nn.Module):
    def __init__(self, feature_dims):
        super(AttFusion_deep, self).__init__()
        self.att = ScaledDotProductAttention(feature_dims)

    def forward(self, xx, record_len, normalized_affine_matrix, copeft_adapter):
        """
        Fusion forwarding.
        ...
        ## tunning for copeft_deep
        out = copeft_adapter(out, record_len, only_adapter=True)
        ##
